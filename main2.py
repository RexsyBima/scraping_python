...
def parsing_var(product_detail, url, catid): #THE PRODUCT_DETAIL MUST BE A DICTIONARY. to parse the model variation
    #print(url)
    ...
    if response == "HTTP Error 400: Bad Request":
        ...
        try:
            ...
            if len(models) > 1:
                ...
                result = None
            return preorder, results, product_status #preorder = return preorder status between 1 and 0 (true or false) #results = sheet2 results...
        except TypeError:
            product_status = 'Not exist'
            return None, None, product_status #product is gone
...
def save_output_folder(shop_name, product_result, processed_data, output_folder="output"):
    ...
    with pd.ExcelWriter(output_file_path, engine='openpyxl') as file:
        df1 = pd.DataFrame(processed_data)
        df1[['catid']] = df1[['catid']].astype(str)
...
def read_xlsx(file_name, output="output"):
    ...
    data1 = pd.read_excel(excel_file_path, sheet_name="Sheet1").to_dict(orient= 'records')
    if len(xlsx.sheet_names) > 1: #to check if the xlsx file has Sheet2 
        data2 = pd.read_excel(excel_file_path, sheet_name="Sheet2").to_dict(orient='records')
        return data1, data2
    else:
        data2 = None
...
def product_and_price_changes_check(shopid, productid, original_list,shop_name, product_url, price_min, price_max, item):
    ...
    if dict_value is not None:
        try:
            ...
            #add_date = list(dict_value.items())
            #date = date_now()
            #add_date.insert(13,(date, item[date])) #the newest date insertion will be always at column N
            #item = dict(add_date) #updating data with latest information
...
def parsing_item_dictionary(item, item_link, preorder,item_price_highest, status):
    return {
...
def parsing(datas):  # <--dictionary expected as input
    ...
    update_datas = []
    #print(shop_name)
    if os.path.exists(os.path.join(os.getcwd(), "output", f"{shop_name}.xlsx")):
    ...
    for item in items:
        ...
        preorder, product_detail, product_status = parsing_var(go_to_product_detail, item_link, item['catid']) #product detail = model variation data/sheet2
        #item_sold = 
    ...
    if not os.path.exists(os.path.join(os.getcwd(), "output", f"{shop_name}.xlsx")):
        save_output_folder(shop_name, product_result, processed_data, output_folder="output") #product_result = sheet2 of a shop, processed_data = sheet1
        ...
        if len(unscraped_datas) > 0:
            ...
            while True:
                try:
                    save_output_folder(shop_name, updated_sheet2, updated_sheet1, output_folder="output")
                    break
            ...
            while True:
                try:
                    save_output_folder(shop_name, sheet_xlsx2, sheet_xlsx1, output_folder="output")
                    break
                except PermissionError:
                    print(f"the {shop_name}.xlsx is still opened, please close the file. restarting in 5 second")
                    time.sleep(5)
        #print(len(unscraped_datas))
        print(f"{item['shop_name']}.xlsx is already there, data xlsx has been updated")
                #save_json(sheet_xlsx1, f"{shop_name}.json")
    #df.        to_excel(f"{item['shop_name']} - documentation test. nxlsx", index=False)
    ...
    for data in unscraped_datas:
        ...
        if match:
            ...
            try:
                ...
                #models = [model for model in result['data']['item']['models']]
                #print(models)
            ...
            continue
        #sheet_xlsx1.insert(data[0], data[1])
        #sheet_xlsx1.pop(data[0]+1)
